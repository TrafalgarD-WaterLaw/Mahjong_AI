...
![Tests](https://img.shields.io/badge/tests-363%20passed-green)
...
- **归属解码层**:四家牌河在画面中央交错、物理不可分,固定区域方案又绑定 UI 坐标无法迁移线下——用**轮转顺序约束**(Turn-order Constraint,谁出牌轮到谁)做全局强先验 + ByteTrack 身份跟踪 + beam search(束宽 16、窗口 10 个出牌事件)联合解码"谁打了什么";证据不足时保留不确定(软归属),新证据到达时用快照回放精确修正历史,而不是推翻重来。
...
> 效果:手牌推断与真实手牌吻合度约 95%(指标口径:真手牌每张牌的推断期望张数均值,完美 1.0;随机基线 ≈ 手牌数/牌池 ≈ 11%);"对手在听牌"的判断与实际相符(说 6 成就是真的约 6 成);对手真听牌时,猜中他听什么牌约三到五成。

**④ 实时性:推理不阻塞主循环**

游戏进行中,检测/推断全部放后台线程,主循环只做轻量事件处理。

> 效果:主循环零卡顿,常规对局端到端决策延迟 120~180ms,检测异常帧时最坏情况有界 < 1s。
...
② 事件提取   牌出现 / 牌河消失 / 手牌数变化 / 副露信号
  ...
  ▼
③ 归属解码   轮转顺序约束 + beam search(软归属 + 冻结 + 翻案)
  │  输出: 每一张牌属于哪一家
  ▼
...
**性能**:单帧识别耗时 86.6ms → 69.6ms(置信度分级精修),画面解析约 14fps,常规对局端到端决策延迟 120~180ms — 详见 [docs/BENCHMARK.md](docs/BENCHMARK.md)(真实对局基准,含逐帧耗时/事件密度)。
