...
对手手牌永远不可见,真实对局没有"标准答案"可核对。因此自建**合成对局模拟器**:四家真手牌全程已知(标准答案),但推理系统只拿到与真实对局相同的出牌事件流——10 局、每局上百个观测点,离线对比"推断结果 vs 真值"。

| 验证什么 | 结果 | 什么意思 |
|---|---|---|
| 认出对手手牌 | **约 95%** | 系统给对手真手牌的权重(随机猜 11%, 完美 100%) |
| 听牌判断 | 数值不虚 | 说"6 成在听"就真的是约 6 成在听 |
| 猜中听什么牌 | 3-5 成 | 真听牌时猜中其等待牌(前三名候选)的比例 |
| 出牌推荐 | 平均向听数更低 | 新算法 vs 旧算法整局 A/B, 离听牌更近 |

局限:模拟玩家的出牌策略与真人不同(真人会防守、失误),结果仅供量化参考;真实对局的结算核对验证已在进行。
