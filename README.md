...
实时视频检测的漏检/误检/抖动不可避免(牌面小、动画遮挡、低帧率)。系统的第一原则:**检测输出不是真相,而是候选信号**。连续帧被抽象成事件(牌出现 / 牌河消失 / 手牌数台阶 / 副露信号),每类事件要求多信号交叉确认才成立——单帧错误最多造成一次事件延迟,不会污染全局状态。

> 效果:即便检测层有漏检误检(recall 74%),对手手牌推断仍达到 0.95 / 1.0(随机瞎猜只有 0.11)——错误被事件流吸收,推断层没有因此崩坏。

**③ 归属是时间问题,不是空间问题**

四家牌河在画面中央交错、物理不可分;且固定区域方案绑定 UI 坐标,无法迁移到线下。解法:**行动者轮转**(谁出牌轮到谁)做全局强先验,ByteTrack 身份跟踪锁定落点身份,beam search 在整条事件序列上联合解码"谁打了什么"。证据不足时保留不确定(软归属),新证据到达时用快照回放精确修正历史,而不是推翻重来。

> 效果:被碰走、漏检等异常事件不产生连锁错位。

**④ 对手手牌是不可观测量的贝叶斯推断**

对手手牌永远看不到,只能从"他打了什么、没打什么"反推。用**粒子滤波**(3 家 × 400 粒子)近似后验分布;关键在观测模型——理性玩家不会打出毁牌效的牌,用"向听数边际价值"建模打出似然;归属不确定时按概率部分更新粒子,而不是赌一个答案。副露内容也用牌池约束反解(碰需要池中剩 ≥2 张,吃需要顺子伙伴各 ≥1)。

> 效果:手牌质量 0.95、听牌概率校准单调、等待牌 top3 命中 33-55%。
